...
## 2025-07-25 - [StrictYAML ANSI Crash Blocks Downstream Key Sanitization]
**Vulnerability:** While adding sanitization for ANSI escape codes injected into YAML keys, it was discovered that `strictyaml` itself contains a bug that causes it to crash with an unhandled internal `AttributeError` ('ReaderError' object has no attribute 'context_mark') when it tries to parse ANSI control characters inside YAML strings. This intercepts the payload before it even reaches custom downstream parsing and sanitization logic, forcing the application into the fallback Exception handler instead of validating the keys as intended.
**Learning:** You cannot depend on custom downstream validation to catch invalid structures (like control codes) if the upstream parser library has internal bugs that cause it to crash on the same inputs. When writing test suites to verify custom validation logic, these upstream library bugs can mask failures or make it impossible to trigger the code paths you intend to test.
**Prevention:** When testing custom sanitization logic for data provided by third-party parsers, mock the parser's output (`strictyaml.load`) if the parser natively crashes on the malformed test payloads. This ensures your downstream application logic is robustly tested and remains safe even if the third-party parser's bugs are fixed in future versions.
