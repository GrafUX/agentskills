...
## 2025-02-14 - Fix CRLF Log Injection vulnerability in strictyaml parsing

**Vulnerability:** The stringified output of `strictyaml.YAMLError` (`str(e)`) was sanitized for ANSI escape codes but preserved newlines (`\n`, `\r`, `\t`). When this error string was later truncated and wrapped in a `ParseError` for downstream logging, an attacker could supply a maliciously crafted YAML file to inject CRLF characters into logs, enabling Log Injection.
**Learning:** `_sanitize_error_text` deliberately preserves safe whitespace which is dangerous if directly used in log statements. Furthermore, fixing this required mitigating it securely and optimally, per memory instructions, utilizing a module-level precomputed `str.maketrans("\n\r\t", "   ")`.
**Prevention:** Always escape or translate formatting characters like `\n`, `\r`, `\t` into spaces from untrusted input before exposing it in exceptions or reflecting it downstream. Precompute the translation table via `str.maketrans` for performance.
