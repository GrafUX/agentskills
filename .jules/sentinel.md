...
**Vulnerability:** When handling `RuntimeError` during file path resolution in `parser.py` and `validator.py`, using `str(e)` leaked absolute server file paths (e.g., symlink loop paths) back to the user via error messages.
