...
from .parser import read_properties, _remove_newlines, _safe_name, _sanitize_error_text
...
def to_prompt(skill_dirs: list[Path]) -> str:
    ...
    for d in skill_dirs:
        ...
        try:
            ...
            sanitized_description = _remove_newlines(
                _sanitize_error_text(props.description)
            )
            sanitized_path = _remove_newlines(
