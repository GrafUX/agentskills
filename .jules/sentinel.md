...
## 2026-07-28 - [CPU/Memory Exhaustion DoS via Late Truncation]
**Vulnerability:** Standardized sanitization and normalization helpers (such as `_safe_name` and `unicodedata.normalize` in `_validate_name`) were processing the full length of massive untrusted input strings (e.g., 100MB) before performing truncation or bounds checks. This allowed attackers to trigger high CPU usage and memory consumption by submitting extremely large invalid strings, causing a Denial of Service (DoS) during error reflection or input validation.
**Learning:** Checking bounds and truncating untrusted inputs must happen *before* executing any complex or CPU-intensive operations (such as regex-based sanitization or Unicode normalization), rather than afterward.
**Prevention:** Implement early fail-fast truncation and limit checks at the very entry point of sanitization and validation functions.
