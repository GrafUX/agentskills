...
## 2025-07-24 - [Type Confusion DoS via Unvalidated Top-Level Metadata]
**Vulnerability:** The `validate_metadata` function in `validator.py` accepted metadata payloads but failed to verify if the input was actually a dictionary before attempting dictionary operations. Additionally, it assumed all keys were strings when performing set subtractions (`set(metadata.keys()) - ALLOWED_FIELDS`) and string joins (`", ".join(...)`). If an attacker (or a bypass of the parser) supplied a non-dictionary (like a string or list), or a dictionary with mixed string and integer keys (e.g., `{123: 'val'}`), it would cause a `TypeError` and crash the application, resulting in a Denial of Service (DoS).
**Learning:** Type enforcement must happen at the entry point of all public API functions, even those primarily used internally, because consumers might bypass upstream parsers. Furthermore, operations like `set` sorting or `.join()` implicitly expect homogeneous types (usually strings), and failing to cast them from untrusted input leads to unhandled exceptions.
**Prevention:** Always enforce explicit type validation (`isinstance(metadata, dict)`) at the entry of validation functions. When manipulating keys from untrusted mappings (like YAML or JSON), explicitly cast them to strings (e.g., `{str(k) for k in metadata.keys()}`) before performing set operations, sorting, or string joining.
