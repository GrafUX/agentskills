...
## 2025-07-22 - [Type Confusion DoS in Dictionary Operations]
**Vulnerability:** The `validate_metadata` function in `skills-ref/src/skills_ref/validator.py` did not validate that the incoming `metadata` argument was a dictionary, nor did it ensure that the keys within the dictionary were strings before performing set operations and joining. An attacker could bypass parser-level validation (or a developer could pass invalid data directly) containing non-string keys like integers (which `strictyaml` can produce), causing `set(metadata.keys()) - ALLOWED_FIELDS` followed by `sorted()` or `", ".join()` to crash with a `TypeError` due to mixed types, leading to a Denial of Service.
**Learning:** Public API functions must validate the types of their arguments, even if internal parsers are expected to provide clean data. Furthermore, performing operations like sorting or set differences on untrusted dictionary keys without ensuring uniform types (e.g., all strings) is a vector for Type Confusion crashes.
**Prevention:** Always explicitly validate the types of incoming arguments to public API functions (e.g., `isinstance(metadata, dict)`). When working with keys from dictionaries parsed from untrusted sources (like YAML), explicitly cast the keys to strings before performing operations that require homogeneous types.
