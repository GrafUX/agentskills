## 2025-07-28 - [Denial of Service via Unicode Normalization on Unvalidated Inputs]
**Vulnerability:** In `validator.py`, `_validate_name` performed `unicodedata.normalize("NFKC", name.strip())` on the raw name string before validating its length. On extremely long maliciously crafted inputs, performing NFKC normalization consumes excessive CPU and memory, exposing the validator to a Denial of Service (DoS) attack.
**Learning:** Functions that perform computationally complex or intensive operations (like Unicode normalization) must never be run on untrusted inputs before enforcing length or size constraints.
**Prevention:** Implement strict length checks as a "fail-fast" step at the very beginning of validation/parsing logic, before invoking expensive third-party or built-in CPU-heavy operations.
