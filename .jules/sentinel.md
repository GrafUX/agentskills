...
## 2025-07-28 - [CRLF Log Injection via YAMLError Messages]
**Vulnerability:** The stringified exception message from `strictyaml.YAMLError` was truncated and embedded into `ParseError` after sanitization for control characters, but it preserved newline, carriage return, and tab characters. An attacker could exploit this by triggering parsing errors with inputs that included `\r` and `\n`, thereby forging log entries or executing Terminal Injection when the error was displayed or logged downstream.
**Learning:** Sanitizing ANSI codes and basic control characters is not enough. Preserving typical whitespace characters like `\n` or `\r` can lead to CRLF injection if the exception message is unescaped when handled by the logging system.
**Prevention:** Explicitly translate `\n`, `\r`, and `\t` into spaces using `str.translate` for any untrusted string reflected in error messages, particularly when stringifying third-party exceptions.
