...
## 2024-05-24 - Fix Log Injection (CRLF) in YAMLError messages
**Vulnerability:** Log Injection (CRLF injection) vulnerability when rendering `strictyaml.YAMLError` error string inside `parse_frontmatter`. The original `_sanitize_error_text` explicitly allowed newlines (`\n`, `\r`) and tabs (`\t`) for readability, leading to unescaped control characters in downstream logs.
**Learning:** Although keeping safe whitespace in error messages seems helpful for output formatting, it introduces log injection vectors when the truncated or full error is logged by external systems consuming the `ParseError`.
**Prevention:** Always replace newlines (`\n`, `\r`) and tabs (`\t`) with spaces before truncating and propagating error messages derived from untrusted parsing payloads.
