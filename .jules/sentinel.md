...
## 2026-07-18 - [Unsanitized Frontmatter Key Reflection in Parser]
**Vulnerability:** In `parser.py`, untrusted dictionary keys parsed from YAML frontmatter were reflected in `ParseError` exceptions as part of `display_key` without sanitizing them. This violated the project's strict error sanitization policy, which requires stripping control characters and ANSI escape sequences from error messages to prevent terminal/log manipulation attacks.
**Learning:** Even though keys must be strings and are validated, they are still untrusted inputs directly derived from external YAML. Directly interpolating them or their sliced representations into error messages introduces vulnerability vectors if they contain malicious formatting or control codes.
**Prevention:** Always sanitize untrusted keys/values before reflecting them in error messages or exception details using helper functions like `_safe_name()` or `_sanitize_error_text()`.
