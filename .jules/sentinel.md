...
## 2024-07-13 - [DoS via `str()` on Untrusted Parsed Input]
**Vulnerability:** Calling `str()` on untrusted nested structures (like parsed YAML fields) to stringify them instead of validating types.
**Learning:** `strictyaml` can sometimes produce complex nested data structures if flow mappings/lists bypass strict checks or if an attacker deeply nests structures. Calling `str(value)` on arbitrary objects is an O(N) operation (or worse) that allocates large strings, potentially causing a Denial of Service (DoS) by consuming excessive CPU and memory on malicious inputs.
**Prevention:** Strictly verify `isinstance(value, str)` (an O(1) operation) *before* attempting string transformations or length checks. Never forcibly cast unvalidated structures to strings when the contract expects a string.
