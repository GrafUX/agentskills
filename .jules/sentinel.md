...
## 2026-07-28 - Log Injection (CRLF) in YAML Parser Errors
**Vulnerability:** Log/CRLF Injection via third-party library error messages (`strictyaml.YAMLError`). The stringified representation of `YAMLError` (`str(e)`) was reflected into the `ParseError` exception message without sanitizing newline (`\n`), carriage return (`\r`), or tab (`\t`) characters. When logged downstream, this could allow an attacker to inject multiline payloads and spoof log entries (Log Injection).
**Learning:** `_sanitize_error_text` explicitly preserves safe whitespace like `\n`, `\r`, and `\t`, which is dangerous when reflecting error strings generated from untrusted inputs into exception messages that will be logged.
**Prevention:** When catching `strictyaml.YAMLError` and stringifying it, explicitly replace newlines (`\n`), carriage returns (`\r`), and tabs (`\t`) with spaces before truncating and embedding it into the downstream `ParseError`.
