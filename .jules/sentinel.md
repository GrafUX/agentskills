...
## 2024-07-16 - Type Confusion DoS in Untrusted Dictionary Iteration
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `validate_metadata` and `_validate_metadata_fields` where bypassing parser-level validation (e.g. by directly passing a string or a dictionary with mixed key types, like strings and integers) caused `AttributeError` or `TypeError` crashes during set operations and sorting.
**Learning:** Even when input is assumed to have been parsed and validated upstream (like YAML frontmatter), public API functions taking loosely typed dictionaries must re-validate the root type and ensure key homogeneity before performing operations that require it (like sorting).
**Prevention:** Always explicitly check the type of untrusted dict arguments (e.g., `isinstance(metadata, dict)`). Additionally, when processing dynamically parsed mappings, explicitly cast keys to strings (`str(k)`) before performing set arithmetic or sort operations to prevent mixed-type crashes.
