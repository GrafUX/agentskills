...
## 2026-07-28 - [Trojan Source and ANSI Injection in YAML Frontmatter Parsing]
**Vulnerability:** While `strictyaml` enforces structure, it happily parses unprintable control characters (such as the bidirectional override character `\u202e`) and ANSI escape sequences in keys and values. Downstream consumers logging or displaying these parsed fields remain fully vulnerable to Terminal/Log Injection and Trojan Source attacks.
**Learning:** Parsing libraries ensure syntax correctness but do not enforce character-level safety for the parsed keys and values. Security validation of parsed fields must run at the ingestion/parsing boundary to prevent malicious characters from permeating downstream systems.
**Prevention:** Always validate that raw frontmatter blocks and parsed keys, values, and custom nested metadata do not contain raw ANSI sequences or unprintable control characters (except safe whitespaces like `\n`, `\r`, and `\t`) before passing them to the application.
