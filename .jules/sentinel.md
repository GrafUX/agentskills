...
## 2025-07-28 - [Log Injection Mitigation via YAMLError Reflection]
**Vulnerability:** In `parser.py`, the `strictyaml.YAMLError` exception string was sanitized for ANSI escapes, but newlines, carriage returns, and tabs were not replaced with spaces before being reflected in the application-level `ParseError`. This allowed an attacker to supply a malicious SKILL.md file with specifically formatted invalid YAML that generated a multiline error. When logged or displayed on the terminal by downstream consumers, these unchecked whitespace characters could be used for log injection (CRLF injection) or confusing terminal output.
**Learning:** Even when stripping dangerous control characters and ANSI sequences, raw whitespaces (especially newlines) in reflected error messages can break log file formats and enable log manipulation attacks. All untrusted input reflected into single-line contexts (like error messages or logs) must have layout-altering whitespace neutralized.
**Prevention:** Always replace newlines (`\n`), carriage returns (`\r`), and tabs (`\t`) with spaces when sanitizing error text or reflecting untrusted stringified objects (like external library exceptions) into application error messages.
