...
## 2024-08-03 - [CRLF Log Injection in YAML Parser Errors]
**Vulnerability:** The stringified YAMLError in `strictyaml.load` error handling was directly passed to `_sanitize_error_text` which preserved unsafe whitespaces (`\n`, `\r`, `\t`), and the resulting un-escaped error message could be used to perform CRLF (Log/Terminal) Injection when logged downstream.
**Learning:** `_sanitize_error_text` was intentionally preserving safe whitespace, but this caused CRLF injection vulnerabilities for error messages since they contain untrusted user input from invalid YAML.
**Prevention:** Explicitly remove or replace `\n`, `\r`, and `\t` with spaces for any untrusted string reflected in error messages, either via a translation table or chained `.replace()` calls.
