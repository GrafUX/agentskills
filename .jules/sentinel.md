...
## 2025-02-14 - Fix Denial of Service via Large File Read
**Vulnerability:** A Denial of Service (DoS) vulnerability via Large File Read was found. Although a hard 1MB file size limit check was enforced, it only occurred *after* attempting to read up to 1MB from the file into memory (`f.read(1024 * 1024 + 1)`). An attacker could still repeatedly force the application to perform relatively large and unnecessary I/O operations and string memory allocations for very large files.
**Learning:** File size constraints should be validated at the metadata level (`st_size`) prior to performing heavy disk I/O or allocating large strings in memory. This "fail fast" approach reduces resource exhaustion.
**Prevention:** Always check a file's metadata (using `path.stat().st_size`) to enforce maximum size bounds *before* calling `open()` and `read()`, keeping the bounded `read()` only as a secondary fallback for virtual files that may misreport their size.
