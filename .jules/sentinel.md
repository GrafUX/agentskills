...
## 2025-06-12 - [Symlink Path Leakage]
**Vulnerability:** Path leakage via RuntimeError when handling symlink loops.
**Learning:** Exception handling for missing or unreadable files can unintentionally leak full system paths if the raw exception or absolute path is formatted into user-facing output.
**Prevention:** Sanitize error messages by relying solely on the final path component (`skill_dir.name`) and generic error messages.
