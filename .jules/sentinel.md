...
## 2026-07-26 - [Log Injection via Unsanitized YAML Exceptions]
**Vulnerability:** In `parser.py`, the stringified exception message `str(e)` from `strictyaml.YAMLError` was truncated and embedded in a `ParseError` without replacing structural whitespace (like newlines and carriage returns). An attacker could provide a malicious SKILL.md file containing unprintable newlines inside invalid YAML syntax, injecting forged log lines when the downstream application logs the `ParseError`.
**Learning:** Even if a utility function like `_sanitize_error_text` strips control characters and ANSI escapes, it might intentionally preserve structural whitespace (like `\n` or `\t`) for standard readability. Such whitespace in error reflections represents a severe log injection (CRLF) vulnerability if the string directly incorporates arbitrary user data (like a YAMLError).
**Prevention:** Always explicitly replace newlines (`\n`), carriage returns (`\r`), and tabs (`\t`) with spaces immediately after base sanitization (`_sanitize_error_text`) when embedding external library exceptions containing user input into application-level error structures.
