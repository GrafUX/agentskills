...
## 2026-07-28 - [XSS Bypass via Control Characters in URL Sanitization]
**Vulnerability:** The `sanitizeUrl` function in JSX components relied solely on regex matching (`/^(https?:\/\/|\/(?!\/))/i`) and `trim()` to prevent `javascript:` XSS. However, `trim()` does not remove all non-printable control characters or specific unicode whitespaces that modern browsers might ignore when executing a URL. An attacker could bypass the protocol allowlist by prefixing a `javascript:` URI with hidden control characters (e.g., `\x01javascript:alert(1)`).
**Learning:** Basic string trimming and regex matching are insufficient for robust URL sanitization against XSS. Browsers are notoriously lenient in parsing URIs and will often ignore or strip various control characters or whitespace variants before execution, potentially allowing malicious payloads to slip past validation logic that expects a strict prefix match.
**Prevention:** Always strip all control characters and whitespaces (e.g., using `.replace(/[\u0000-\u001F\u007F-\u009F\s]+/g, '')`) from untrusted URL strings *before* applying protocol validation or allowlist regexes.
