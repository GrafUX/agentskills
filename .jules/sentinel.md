...
## 2026-07-27 - [CPU Exhaustion DoS via Unbounded Unicode Normalization]
**Vulnerability:** In `_validate_name`, the function called `unicodedata.normalize("NFKC", ...)` directly on user-provided strings before checking if the name exceeded standard limits. An attacker supplying a massive (e.g. multi-megabyte) string under the `name` field in SKILL.md could exhaust system CPU resources during validation, leading to a Denial of Service (DoS).
**Learning:** Unicode normalization functions like `unicodedata.normalize` are computationally expensive and scale non-linearly with string length. Running normalization on untrusted, unbounded inputs before validating string length exposes the application to resource exhaustion.
**Prevention:** Implement strict length checks at the very entry of validation functions, failing fast if the untrusted input exceeds maximum limits (e.g. `MAX_SKILL_NAME_LENGTH`) before executing expensive operations like Unicode normalization.
