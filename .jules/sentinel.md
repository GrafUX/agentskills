...
## 2026-07-25 - [CRLF Injection via strictyaml YAMLError Messages]
**Vulnerability:** The stringified exception message `str(e)` from `strictyaml.YAMLError` was passed to `_sanitize_error_text` (which retains safe whitespace like `\n`) and directly embedded into a `ParseError` without removing newlines. An attacker could provide malicious YAML containing newlines within error contexts, leading to CRLF (Log) Injection when the resulting error is logged.
**Learning:** Exception messages that incorporate untrusted input must not only have control characters stripped, but also have formatting whitespaces (newlines, carriage returns, tabs) neutralized if they are going to be safely truncated and reflected in downstream logs or terminal outputs.
**Prevention:** Always apply `_remove_newlines` (using `str.translate` for efficiency) after `_sanitize_error_text` before truncating or reflecting third-party exception strings.
