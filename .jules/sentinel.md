...
## 2025-07-28 - [Log Injection via Unsanitized Whitespace in Error Messages]
**Vulnerability:** The exception error messages in `parser.py` preserved whitespaces like newlines (`\n`), carriage returns (`\r`), and tabs (`\t`) when catching `strictyaml.YAMLError`. If an attacker provided a SKILL.md with a maliciously crafted, multi-line error string, the error thrown by the library would preserve the multi-line payload. This leads to a Log Injection / CRLF Injection vulnerability if downstream services log this unsanitized error message.
**Learning:** Even if ANSI escape sequences are stripped, standard whitespace characters can be weaponized to forge log entries or manipulate log formats (e.g. inserting new, fake log lines).
**Prevention:** Always flatten multi-line error messages derived from untrusted inputs by replacing newlines, carriage returns, and tabs with spaces before reflecting them into exception classes or logs.
