...
## 2026-07-27 - [CRLF Log Injection via Preserved Safe Whitespace]
**Vulnerability:** First, raw input whitespaces (like newlines, carriage returns, or tabs) reflected in exception/error messages could lead to log/terminal injection.
**Learning:** Generic sanitization functions (like `_sanitize_error_text`) often preserve "safe" whitespace such as `\n`, `\r`, and `\t` because it is generally acceptable for UI display. However, when these preserved whitespaces are subsequently embedded into a stringified exception message (e.g., `str(e)`) and raised as an application-level error (like `ParseError`), downstream components logging these errors become vulnerable to Log Injection (CRLF injection).
**Prevention:** Always ensure that downstream use cases specifically requiring single-line outputs (such as log entries or truncated error reflections) explicitly strip or replace preserved whitespaces (e.g., `err_msg.translate(WHITESPACE_TRANS)`) before raising the exception.
