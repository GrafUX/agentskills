...
## 2025-02-28 - [Stringification DoS via Custom Metadata]
**Vulnerability:** The SKILL.md frontmatter parser allowed untrusted yaml `metadata` fields containing arbitrary nested dictionaries and lists to bypass type validation and be stringified using `str(v)`. This stringification of unchecked arbitrary depth structures poses a Denial of Service (DoS) vector by consuming excessive resources.
**Learning:** The root issue stemmed from a blanket assumption that all YAML parsing outcomes within a dictionary block could be cleanly transformed into string pairs `str(k): str(v)` without first checking their structural complexity.
**Prevention:** Always validate parsed objects prior to stringifying them in untrusted data structures, and reject complex data structures (lists/dicts) for plain-text value fields early in the parsing phase by checking `isinstance(v, (dict, list))`.
