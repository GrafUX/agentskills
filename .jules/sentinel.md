...
## 2024-07-25 - Prevent DoS via complex YAML structures in skills-ref parser
**Vulnerability:** Untrusted YAML frontmatter input allowed complex structures (lists, nested dicts) in fields expected to be strings (`license`, `allowed-tools`, `compatibility`, `metadata` values). This could lead to stringification DoS or unhandled TypeErrors downstream.
**Learning:** `strictyaml` does not inherently block complex structures if not validated explicitly against a strict schema. The parser function `parse_frontmatter` converted dicts to string representation `str(v)` directly which can be dangerous for deep dicts.
**Prevention:** Always validate `isinstance(v, str)` for simple fields, and explicitly block `(dict, list)` before applying `str()` on custom metadata dictionary values.
