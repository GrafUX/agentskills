...
from .parser import _safe_name, _sanitize_error_text, find_skill_md, read_properties
...
def to_prompt(skill_dirs: list[Path]) -> str:
    ...
    for skill_dir in skill_dirs:
        try:
            ...
            error_msg = (
                _sanitize_error_text(str(e.strerror))
                if hasattr(e, "strerror")
                else "Symlink loop or unresolvable path"
            )
            raise SkillError(
                f"Failed to resolve skill directory {_safe_name(Path(skill_dir).name)}: {error_msg}"
            )
    ...
    for skill_dir in unique_skill_dirs:
        try:
            ...
            raise SkillError(
                f"Failed to read properties for {_safe_name(Path(skill_dir).name)}: {error_msg}"
            )
