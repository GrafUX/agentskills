...
## 2026-07-28 - [Log Injection via CRLF in Exception Messages]
**Vulnerability:** In `parser.py`, the stringified exception message `str(e)` from `strictyaml.YAMLError` was truncated and embedded in a `ParseError` after having ANSI codes and unprintable characters removed, but it retained standard whitespace characters like newlines, tabs, and carriage returns (`\n`, `\t`, `\r`). An attacker could provide a malicious SKILL.md file containing unexpected YAML errors laden with multiple newlines and carriage returns. When the application caught and logged the error, these unsanitized characters could lead to Log Injection / CRLF Injection, manipulating log file structures and obscuring attacks.
**Learning:** Functions that sanitize input (like `_sanitize_error_text`) may preserve seemingly innocuous formatting characters (like `\n`) which are still dangerous if the string is later logged into line-based text logs.
**Prevention:** Explicitly translate or replace newlines, carriage returns, and tabs with spaces (e.g., using `str.maketrans` and `.translate()`) in error reflections *after* initial sanitization, especially when the error text incorporates unpredictable library output.
