...
## 2024-06-28 - [Log Manipulation and ANSI Injection in Validator Error Message Reflection]
**Vulnerability:** Unexpected/extra frontmatter keys in `skills-ref/src/skills_ref/validator.py` were reflected directly in the validation error output without ANSI sanitization or control character removal. An attacker supplying a maliciously formatted `SKILL.md` file containing ANSI escape sequences or control codes in key names could manipulate console output, pollute logs, or trigger local terminal behavior.
**Learning:** Any fields or keys parsed from external metadata files must be treated as untrusted. If they are validated and subsequently reflected back in error messages, they must be sanitized of ANSI sequences and control characters.
**Prevention:** Sanitize unexpected/extra frontmatter keys before reflecting them in validation error messages using `_sanitize_error_text` to strip ANSI escape sequences and non-printable control characters.
