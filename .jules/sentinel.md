...
## 2026-07-25 - [Log Injection via CRLF in Exception Messages]
**Vulnerability:** Untrusted newlines, carriage returns, and tabs from `strictyaml.YAMLError` stringified messages were truncated and wrapped in a `ParseError` without being sanitized in `parser.py`. This allowed malicious YAML files to introduce CRLF injections in error messages, polluting downstream logs.
**Learning:** `_sanitize_error_text` deliberately preserves safe whitespace such as newlines to render multi-line errors correctly in some scenarios, but before truncating and concatenating such texts in a single-line error reflection context, these characters must be explicitly handled (e.g. replaced with spaces) to prevent terminal and log injection (CRLF). Replacing whitespace efficiently in Python for strings with no matches or multiple matches can be optimized with `str.translate` instead of chained `.replace()` calls.
**Prevention:** Replace raw whitespace characters (`\n`, `\r`, `\t`) in stringified error messages using an optimized `str.translate` table (`str.maketrans`) before truncating and propagating them, mitigating both log injection vectors and performance overheads.
