...
## 2026-08-06 - [CRLF Log Injection via Preserved Whitespace in Error Messages]
**Vulnerability:** In `parser.py`, `_sanitize_error_text()` stripped ANSI escape sequences but intentionally preserved whitespace characters like `\n`, `\r`, and `\t`. When stringified `strictyaml.YAMLError` exceptions containing invalid YAML input were parsed, these newline characters were kept. `parse_frontmatter` embedded this string into a `ParseError` without converting the newlines to spaces. When logged downstream, an attacker could inject CRLF sequences to manipulate logs.
**Learning:** Functions designed to sanitize string values for terminal/log output must not just strip dangerous control characters; they must also flatten structure (replacing newlines/tabs with spaces) if the resulting string is going to be embedded in a single-line log or exception message where CRLF injection is a risk.
**Prevention:** Apply a whitespace translation table (`str.maketrans("\n\r\t", "   ")`) to any sanitized string before it is embedded in exception or error messages, as `strictyaml` or other libraries may reflect the exact user input with newlines intact.
