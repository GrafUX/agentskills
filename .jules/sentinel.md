...
## 2025-03-01 - Type Confusion Denial of Service
**Vulnerability:** In skills-ref's metadata validation (`validate_metadata`), passing untrusted inputs could trigger unhandled exceptions (Type Confusion). Passing a non-dictionary caused an `AttributeError` on `.keys()`, and passing mixed-type keys (e.g. ints and strings) caused a `TypeError` during sorting and string joining operations.
**Learning:** These types of exceptions can lead to unexpected crashes, exposing the application to DoS attacks when processing externally-provided data, and potentially leaking stack traces.
**Prevention:** Always validate top-level structures of untrusted inputs explicitly (`isinstance(metadata, dict)`) before interacting with them, and explicitly cast dictionary keys to strings (`str(k)`) before performing set operations, sorts, or joining.
