...
## 2024-08-12 - [MEDIUM] CRLF Injection in YAML parser
**Vulnerability:** Log Injection / CRLF Injection where malicious YAML could include newline characters (`\n`, `\r`) in strictlyaml.YAMLError strings, allowing log spoofing if reflected downstream.
**Learning:** `_sanitize_error_text` explicitly preserves newlines for readability. However, `parse_frontmatter` truncated the `err_msg` directly and raised `ParseError` without removing these whitespace characters, meaning unexpected newlines could end up in terminal outputs and logs.
**Prevention:** Unconditionally replace safe but structurally dangerous whitespace characters (`\n`, `\r`, `\t`) with spaces using an efficient translation table like `_remove_newlines` before concatenating arbitrary user-provided YAML strings into generic error exceptions.
