...
## 2025-07-25 - [Length Limit Bypass via YAML Types]
**Vulnerability:** The SKILL.md parser enforced the `MAX_FRONTMATTER_VALUE_LENGTH` limit only if the value was parsed as a string (`isinstance(value, str)`). Because `strictyaml` can parse fields as lists or dicts, an attacker could supply an extremely large nested list or dict in fields like `allowed-tools` or `metadata`. This bypassed the length check during parsing. Later, when these values were stringified or processed, they could cause memory resource exhaustion (Prompt Inflation / DoS).
**Learning:** Type-checking before applying length checks on dynamically typed configurations (like YAML) allows non-string types to evade those limits. When parsing YAML, the data structures can be lists or dictionaries whose serialized length is massive.
**Prevention:** Always check the length of the string representation (`len(str(value))`) for dynamically typed config values before accepting them, or strictly enforce type validation (rejecting non-strings) before checking lengths.
